...
export async function getNNewestGames(n: number): Promise<GameTable[]> {
  if (!n) throwErrorException(`[service.game.getNNewestGames] Provide amount of games`, 'Invalid Amount', 400);
  if (n < 1 || n > 15) throwErrorException(`[service.game.getNNewestGames] N must be greater than 0 and less then 15`, 'Invalid Amount', 400);
  return gameRepo.getNNewestGames(n);
}
