...
export async function deleteArticle(article_id: string, article_owner: string | null, admin_id: string | null): Promise<void> {
  ...
  if (admin_id && validate(admin_id)) {
    if (!(await adminRepo.getAdminById(admin_id)))
      throwErrorException(`[service.articles.deleteArticle] Admin ID invalid: ${admin_id}`, 'Admin ID invalid', 403);
    ...
    if (!(await articleRepo.verifyArticleOwnership(article_owner, article_id)))
      throwErrorException(`[service.articles.deleteArticle] User does not own article: ${article_id}`, 'User ID and Article ID mismatch', 403);
    else articleRepo.deleteArticle(article_id);
...
export async function getNNewestArticles(n: number): Promise<ArticleTable[]> {
  if (!n) throwErrorException(`[service.game.getNNewestArticles] Provide amount of Articles`, 'Invalid Amount', 400);
  if (n < 1 || n > 15) throwErrorException(`[service.game.getNNewestArticles] N must be greater than 0 and less then 15`, 'Invalid Amount', 400);
  return articleRepo.getNNewestArticles(n);
}
