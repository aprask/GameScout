...
export async function getWishlistById(wishlist_id: string): Promise<WishlistTable> {
    const wishlist = await db
        ...
        .executeTakeFirst();
    if (!wishlist || wishlist === undefined) throwErrorException(`[repository.wishlist.getWishlistById] cannot find wishlist with ID ${wishlist_id}`, 'Wishlist not found', 404);
    return wishlist!;
}
...
export async function createWishlist(wishlist: WishlistTable): Promise<WishlistTable> {
    const newWishlist = await db
        ...
        .returningAll()
        .executeTakeFirst();
    if (!newWishlist || newWishlist === undefined) throwErrorException(`[repository.wishlist.createWishlist] cannot create wishlist`, 'Cannot create wishlist', 500);
    return newWishlist!;
}
...
export async function updateWishlist(wishlist_id: string, wishlist: Omit<WishlistTable, 'wishlist_id' | 'created_at' | 'updated_at'>): Promise<WishlistTable> {
    const updatedWishlist = await db
        ...
        .returningAll()
        .executeTakeFirst();
    if (!updatedWishlist || updatedWishlist === undefined) throwErrorException(`[repository.wishlist.updateWishlist] cannot update wishlist`, 'Cannot update wishlist', 500);
